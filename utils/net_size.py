# reference : https://github.com/1adrianb/pytorch-estimate-flops/blob/master/pthflops/ops.py
import re
...
def count_ops(model, input, print_readable=False, *args):
    r"""Estimates the number of FLOPs of an :class:`torch.nn.Module`
    :param model: the :class:`torch.nn.Module`
    :param input: a N-d :class:`torch.tensor` containing the input to the models
    :param print_readable: boolean, if True will print the number of FLOPs. default is True
    :return: number of FLOPs
    :rtype: `int`
    """
    # Make sure that the input is on the same device as the models
    if next(model.parameters()).devices != input.devices:
        input.to(next(model.parameters()).devices)
    ...
    return "{:,.2f} GFLOPs, for input size : {}".format(ops / 1e+9, tuple(input.shape))
...
def get_net_size(net):
    ...
    for i in params:
        ...
        for j in i.size():
            l *= j
        k = k + l
    s = ("Total parameters : {:} float, models size : {:.4f}M".format(k, k * 4 / 1024 / 1024))
    return s