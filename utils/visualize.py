...
LightColors = ["#f89588", "#7cd6cf", "#9192ab", "#7898e1", "#efa666", "#9987ce", "#63b2ee", "#76da91", "#63b2ee", "#f8cb7f", ]
...
def set_common_params(x_label=None, y_label=None, title=None, ylim=None, title_size=None,
    ...
    if legend:
        plt.legend(loc=legenc_loc, fontsize=legend_size)
    plt.tight_layout()
    if save_name is not None:
        plt.savefig(save_name, bbox_inches='tight')
    if show:
...
def line_plot(data, x_axis_names=None, line_names=None,
              ...
              text_x_offset=-0.15, text_y_offset=0.05, colors=None,
              x_label=None, y_label=None, title=None, ylim=None, yticks=None, figsize=(8, 5),
              title_size=None, label_size=None, tick_size=None, legenc_loc=None, legend_size=None, ytick_format_n=None,
    ...
    for i, (line_data, style, name, color) in enumerate(zip(data, line_styles, line_names, colors)):
        plt.scatter(x_axis, line_data, color=color, s=marker_size)
        plt.plot(x_axis, line_data, linestyle=style, label=name, color=color, linewidth=line_width)
        for x, y in zip(x_axis, line_data):
            if plot_text:
